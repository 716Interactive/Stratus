...
object PterodactylService {
    ...
    suspend fun createServer(
        ...
        val payload = buildJsonObject {
            ...
            put("user", userId)
            put("egg", eggId)
            // Sanitize Docker image name: lowercase, replace spaces with hyphens (Docker requires lowercase repo names)
            val sanitizedImage = image.lowercase().replace("\\s+".toRegex(), "-")
            put("docker_image", sanitizedImage)
            put("startup", startup)
